...
Consider a linear classifier $h(x) = sign(\dot{w^T, x})$, where $h \in \{-1,+1\}$. 

If we add a small perturbation $\eta$  to input $x$ to create a new input $\tilde{x} = x + \eta$ , we expect the classifier to assign the same label to this new input as long as the small perturbations are smaller than the precision(smallest possible change that can be detected in the data) of the inputs. That is we expect
